...
const Agentes = () => {
  ...
  const cargarAgentes = async () => {
    try {
      ...
      const respuesta = await axios.get(`http://127.0.0.1:8000/agentes/?mostrar_inactivos=${mostrarInactivos}`, {
  ...
  const handleRowClick = (ag) => {
    if (selectedId === ag.id_agente) {
      setSelectedId(null);
      setFormData(estadoInicial);
    } else {
      setSelectedId(ag.id_agente);
      setFormData({
        ruc: ag.ruc || '',
        nombre: ag.nombre || ''
      });
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      ...
      if (viewMode === 'NUEVO') {
        await axios.post('http://127.0.0.1:8000/agentes/', payload, config);
        setMensaje({ texto: '✅ Agente registrado exitosamente.', tipo: 'exito' });
      } else if (viewMode === 'MODIFICAR') {
        await axios.put(`http://127.0.0.1:8000/agentes/${selectedId}`, payload, config);
        setMensaje({ texto: '✅ Agente actualizado exitosamente.', tipo: 'exito' });
      }
  ...
  const handleEliminar = async () => {
    const agenteSeleccionado = agentes.find(a => a.id_agente === selectedId);
    if (!agenteSeleccionado) return;

    const confirmar = window.confirm(`¿Estás seguro de desactivar el agente "${agenteSeleccionado.nombre}"?\nEsta acción lo marcará como INACTIVO.`);
    if (!confirmar) return;

    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://127.0.0.1:8000/agentes/${selectedId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMensaje({ texto: `✅ Agente "${agenteSeleccionado.nombre}" desactivado correctamente.`, tipo: 'exito' });
      cargarAgentes();
      setSelectedId(null);
      setFormData(estadoInicial);
      setTimeout(() => setMensaje({ texto: '', tipo: '' }), 4000);
  ...
  const handleRestaurar = async () => {
    const agenteSeleccionado = agentes.find(i => i.id_agente === selectedId);
    if (!agenteSeleccionado) return;

    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://127.0.0.1:8000/agentes/${selectedId}/restaurar`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMensaje({ texto: `✅ Agente "${agenteSeleccionado.nombre}" restaurado correctamente.`, tipo: 'exito' });
      cargarAgentes();
      setSelectedId(null);
      setFormData(estadoInicial);
      setTimeout(() => setMensaje({ texto: '', tipo: '' }), 4000);
  ...
  const handleEliminarFisico = async () => {
    ...
    const confirmar = window.confirm(`⚠️ ¡ATENCIÓN! ⚠️\n¿Estás seguro de ELIMINAR DEFINITIVAMENTE el agente "${agenteSeleccionado.nombre}"?\nEsta acción NO se puede deshacer y fallará si el registro ya está en uso.`);
    if (!confirmar) return;

    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://127.0.0.1:8000/agentes/${selectedId}/fisico`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMensaje({ texto: `✅ Agente "${agenteSeleccionado.nombre}" eliminado definitivamente.`, tipo: 'exito' });
      cargarAgentes();
      setSelectedId(null);
      setFormData(estadoInicial);
      setTimeout(() => setMensaje({ texto: '', tipo: '' }), 4000);
  ...
  const agentesFiltrados = agentes.filter((ag) =>
    ...
    (ag.ruc || '').includes(busqueda)
  );

  const isReadOnly = viewMode === 'CONSULTAR';
  const agenteActualmenteSeleccionado = agentes.find(a => a.id_agente === selectedId);
  const esInactivoSeleccionado = agenteActualmenteSeleccionado?.estado_registro === 'INACTIVO';
  ...
  return (
    <div style={containerStyle}>
      <h2 style={{ color: '#2c3e50', borderBottom: '2px solid #3498db', paddingBottom: '10px' }}>Maestro de Agentes de Aduanas</h2>
      ...
      {viewMode === 'LISTA' ? (
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
            <h4 style={{ color: '#7f8c8d', margin: 0 }}>Agentes Registrados ({agentesFiltrados.length})</h4>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: '#ecf0f1', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '12px', borderBottom: '2px solid #ddd' }}>Nombre del Agente</th>
              ...
              <tbody>
                {agentesFiltrados.length === 0 ? (
                  <tr>
                    <td colSpan="4" style={{ padding: '15px', textAlign: 'center', color: '#7f8c8d' }}>No hay Agentes registrados aún.</td>
                  </tr>
                ) : (
                  agentesFiltrados.map((ag) => {
                    const esInactivo = ag.estado_registro === 'INACTIVO';
                    return (
                      <tr 
                        key={ag.id_agente} 
                        onClick={() => handleRowClick(ag)}
                        style={{ 
                          backgroundColor: selectedId === ag.id_agente ? '#d6eaf8' : (esInactivo ? '#f9f9f9' : 'white'), 
                          color: esInactivo ? '#95a5a6' : 'inherit',
                          textDecoration: esInactivo && selectedId !== ag.id_agente ? 'line-through' : 'none',
                          cursor: 'pointer', 
                      ...
                      >
                        ...
                        <td style={{ padding: '12px', fontWeight: 'bold' }}>{ag.ruc}</td>
                        <td style={{ padding: '12px' }}>{ag.nombre}</td>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
          <h3 style={{ marginTop: 0, color: '#34495e', borderBottom: '1px solid #eee', paddingBottom: '10px' }}>
            {viewMode === 'NUEVO' ? 'Registro de Nuevo Agente' : viewMode === 'MODIFICAR' ? 'Modificar Agente' : 'Detalle del Agente'}
          </h3>
