...
const Bancos = () => {
  ...
  const cargarBancos = async () => {
    try {
      ...
      const respuesta = await axios.get(`http://127.0.0.1:8000/bancos/?mostrar_inactivos=${mostrarInactivos}`, {
  ...
  // --- MANEJO DE SELECCIÓN ---
  const handleRowClick = (banco) => {
    if (selectedId === banco.id_banco) {
      setSelectedId(null);
      setFormData(estadoInicial);
    } else {
      setSelectedId(banco.id_banco);
      setFormData({
        nombre: banco.nombre || ''
      });
    }
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      ...
      if (viewMode === 'NUEVO') {
        await axios.post('http://127.0.0.1:8000/bancos/', payload, config);
        setMensaje({ texto: '✅ Banco registrado exitosamente.', tipo: 'exito' });
      } else if (viewMode === 'MODIFICAR') {
        await axios.put(`http://127.0.0.1:8000/bancos/${selectedId}`, payload, config);
        setMensaje({ texto: '✅ Banco actualizado exitosamente.', tipo: 'exito' });
      }
  ...
  const handleEliminar = async () => {
    const bancoSeleccionado = bancos.find(b => b.id_banco === selectedId);
    if (!bancoSeleccionado) return;

    const confirmar = window.confirm(`¿Estás seguro de desactivar el banco "${bancoSeleccionado.nombre}"?\nEsta acción lo marcará como INACTIVO.`);
    if (!confirmar) return;

    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://127.0.0.1:8000/bancos/${selectedId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMensaje({ texto: `✅ Banco "${bancoSeleccionado.nombre}" desactivado correctamente.`, tipo: 'exito' });
      cargarBancos();
      setSelectedId(null);
      setFormData(estadoInicial);
      setTimeout(() => setMensaje({ texto: '', tipo: '' }), 4000);
  ...
  const handleRestaurar = async () => {
    const bancoSeleccionado = bancos.find(i => i.id_banco === selectedId);
    if (!bancoSeleccionado) return;

    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://127.0.0.1:8000/bancos/${selectedId}/restaurar`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMensaje({ texto: `✅ Banco "${bancoSeleccionado.nombre}" restaurado correctamente.`, tipo: 'exito' });
      cargarBancos();
      setSelectedId(null);
      setFormData(estadoInicial);
      setTimeout(() => setMensaje({ texto: '', tipo: '' }), 4000);
  ...
  const handleEliminarFisico = async () => {
    ...
    const confirmar = window.confirm(`⚠️ ¡ATENCIÓN! ⚠️\n¿Estás seguro de ELIMINAR DEFINITIVAMENTE el banco "${bancoSeleccionado.nombre}"?\nEsta acción NO se puede deshacer y fallará si el registro ya está en uso.`);
    if (!confirmar) return;

    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://127.0.0.1:8000/bancos/${selectedId}/fisico`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMensaje({ texto: `✅ Banco "${bancoSeleccionado.nombre}" eliminado definitivamente.`, tipo: 'exito' });
      cargarBancos();
      setSelectedId(null);
      setFormData(estadoInicial);
      setTimeout(() => setMensaje({ texto: '', tipo: '' }), 4000);
  ...
  const bancoActualmenteSeleccionado = bancos.find(b => b.id_banco === selectedId);
  const esInactivoSeleccionado = bancoActualmenteSeleccionado?.estado_registro === 'INACTIVO';
  ...
  return (
    <div style={containerStyle}>
      <h2 style={{ color: '#2c3e50', borderBottom: '2px solid #3498db', paddingBottom: '10px' }}>Maestro de Bancos</h2>
      ...
      {viewMode === 'LISTA' ? (
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
            <h4 style={{ color: '#7f8c8d', margin: 0 }}>Bancos Registrados ({bancosFiltrados.length})</h4>
            ...
            <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
              ...
              <input
                ...
                style={{ padding: '8px 12px', border: '1px solid #ccc', borderRadius: '4px', width: '280px', fontSize: '14px' }}
              />
            </div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: '#ecf0f1', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '12px', borderBottom: '2px solid #ddd' }}>Nombre del Banco</th>
              ...
              <tbody>
                {bancosFiltrados.length === 0 ? (
                  <tr>
                    <td colSpan="3" style={{ padding: '15px', textAlign: 'center', color: '#7f8c8d' }}>No hay Bancos registrados aún.</td>
                  </tr>
                ) : (
                  bancosFiltrados.map((banco) => {
                    const esInactivo = banco.estado_registro === 'INACTIVO';
                    return (
                      <tr 
                        key={banco.id_banco} 
                        onClick={() => handleRowClick(banco)}
                        style={{ 
                          backgroundColor: selectedId === banco.id_banco ? '#d6eaf8' : (esInactivo ? '#f9f9f9' : 'white'), 
                          color: esInactivo ? '#95a5a6' : 'inherit',
                          textDecoration: esInactivo && selectedId !== banco.id_banco ? 'line-through' : 'none',
                          cursor: 'pointer', 
                          borderBottom: '1px solid #eee', 
                          transition: 'background-color 0.2s' 
                        }}
                      >
                        <td style={{ padding: '12px' }}>{banco.id_banco}</td>
                        <td style={{ padding: '12px', fontWeight: 'bold' }}>{banco.nombre}</td>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
          <h3 style={{ marginTop: 0, color: '#34495e', borderBottom: '1px solid #eee', paddingBottom: '10px' }}>
            {viewMode === 'NUEVO' ? 'Registro de Nuevo Banco' : viewMode === 'MODIFICAR' ? 'Modificar Banco' : 'Detalle del Banco'}
          </h3>
